...
%% ------------------------------------------------------------------------
% Precomputed geometry/masks for optimized Local Inertial D4 solver
% Built once because these quantities depend only on static terrain/domain
% -------------------------------------------------------------------------
LI_Precomputed = struct();

% Basic dimensions
LI_Precomputed.ny = ny;
LI_Precomputed.nx = nx;

% Static bed-face elevations (depend only on z / elevation)
LI_Precomputed.zf_x = max(Elevation_Properties.elevation_cell(:,1:end-1), ...
    Elevation_Properties.elevation_cell(:,2:end));

LI_Precomputed.zf_y = max(Elevation_Properties.elevation_cell(1:end-1,:), ...
    Elevation_Properties.elevation_cell(2:end,:));

% Static cell validity mask
% True = active computational cell
LI_Precomputed.active_mask = ~idx_nan;

% Static valid-face masks
% X-face between cell(:,j) and cell(:,j+1)
LI_Precomputed.valid_x = (~idx_nan(:,1:end-1)) & (~idx_nan(:,2:end));

% Y-face between cell(i,:) and cell(i+1,:)
LI_Precomputed.valid_y = (~idx_nan(1:end-1,:)) & (~idx_nan(2:end,:));

% Optional outlet linear indices for faster outlet critical-slope update
if ~isempty(Wshed_Properties.row_outlet) && ~isempty(Wshed_Properties.col_outlet)
    LI_Precomputed.outlet_linear_index = sub2ind([ny, nx], Wshed_Properties.row_outlet(:), Wshed_Properties.col_outlet(:));
else
    LI_Precomputed.outlet_linear_index = [];
end

% Debug flag for internal volume check inside Local_Inertial_Model_D4
% Keep false for production speed
LI_Precomputed.compute_error_vol = false;

% -------------------------------------------------------------------------
% Precomputed face-based Manning roughness for Local Inertial D4
%   (:,:,1) -> x-faces
%   (:,:,2) -> y-faces
% -------------------------------------------------------------------------
LI_Precomputed.n_face = zeros(ny,nx,2,'like',LULC_Properties.roughness);

% x-faces: average neighboring cells in x-direction
LI_Precomputed.n_face(:,1:end-1,1) = 0.5 .* ...
    (LULC_Properties.roughness(:,1:end-1) + LULC_Properties.roughness(:,2:end));
LI_Precomputed.n_face(:,end,1) = LULC_Properties.roughness(:,end);

% y-faces: average neighboring cells in y-direction
LI_Precomputed.n_face(2:end,:,2) = 0.5 .* ...
    (LULC_Properties.roughness(1:end-1,:) + LULC_Properties.roughness(2:end,:));
LI_Precomputed.n_face(1,:,2) = LULC_Properties.roughness(1,:);

LI_Precomputed.n_face_sq = LI_Precomputed.n_face .^ 2;

LI_Precomputed.valid_x_full = false(ny,nx,'like',idx_nan);
LI_Precomputed.valid_x_full(:,1:nx-1) = LI_Precomputed.valid_x;

LI_Precomputed.valid_y_full = false(ny,nx,'like',idx_nan);
LI_Precomputed.valid_y_full(2:ny,:) = LI_Precomputed.valid_y;

LI_Precomputed.mask_outlet_cells = false(ny,nx,'like',idx_nan);
if ~isempty(LI_Precomputed.outlet_linear_index)
    LI_Precomputed.mask_outlet_cells(LI_Precomputed.outlet_linear_index) = true;
end

% Zeros
LI_Precomputed.Zeros = zeros(ny,nx,'like',depths.d_t);
LI_Precomputed.Zeros_3D = zeros(ny,nx,3,'like',depths.d_t);

% % Fluxes
% LI_Work.Vol_Flux   = zeros(ny,nx,'like',depths.d_t);
% LI_Work.qout_left  = zeros(ny,nx,'like',depths.d_t);
% LI_Work.qout_right = zeros(ny,nx,'like',depths.d_t);
% LI_Work.qout_up    = zeros(ny,nx,'like',depths.d_t);
% LI_Work.qout_down  = zeros(ny,nx,'like',depths.d_t);

%% ------------------------------------------------------------------------
% Reusable work arrays for Local_Inertial_Model_D4
% -------------------------------------------------------------------------
LI_Work = struct();

% Main numeric work arrays
LI_Work.Sx           = zeros(ny,nx,'like',depths.d_t);
LI_Work.Sy           = zeros(ny,nx,'like',depths.d_t);
LI_Work.Hf           = zeros(ny,nx,3,'like',depths.d_t);
LI_Work.matrix_store = zeros(ny,nx,3,'like',depths.d_t);
LI_Work.outflow      = zeros(ny,nx,3,'like',depths.d_t);

% Volume / directional flux work arrays
LI_Work.Vol_Flux   = zeros(ny,nx,'like',depths.d_t);
LI_Work.qout_left  = zeros(ny,nx,'like',depths.d_t);
LI_Work.qout_right = zeros(ny,nx,'like',depths.d_t);
LI_Work.qout_up    = zeros(ny,nx,'like',depths.d_t);
...
% Outlet work arrays
LI_Work.S_0        = zeros(ny,nx,'like',depths.d_t);
LI_Work.Hf_outlet  = zeros(ny,nx,'like',depths.d_t);

% Full-size logical masks reused every time step
LI_Work.finite_x_full  = false(ny,nx);
LI_Work.finite_y_full  = false(ny,nx);
LI_Work.dry_x_full     = false(ny,nx);
LI_Work.dry_y_full     = false(ny,nx);
LI_Work.shallow_x_full = false(ny,nx);
LI_Work.shallow_y_full = false(ny,nx);
LI_Work.mask_x_full    = false(ny,nx);
LI_Work.mask_y_full    = false(ny,nx);

% Constant coarse-cell area raster
LI_Precomputed.C_a_full = Wshed_Properties.Resolution^2 * ones(ny,nx,'like',depths.d_t);

LI_Precomputed.face_invalid = false(ny,nx,2,'like',idx_nan);
LI_Precomputed.face_invalid(:,1:end-1,1) = idx_nan(:,1:end-1) | idx_nan(:,2:end);
LI_Precomputed.face_invalid(:,end,1)     = idx_nan(:,end);
LI_Precomputed.face_invalid(2:end,:,2)   = idx_nan(1:end-1,:) | idx_nan(2:end,:);
LI_Precomputed.face_invalid(1,:,2)       = idx_nan(1,:);
...
while t <= (running_control.routing_time + running_control.min_time_step/60) % Running up to the end of the simulation
    ...
    try
        tic
        % -------------- Hydrological Model --------------- %
        % BC_States.delta_p_agg(BC_States.delta_p_agg >= 0) = 10* (time_step/60); % 1 mm/h
        % time_step = 1; % min

        Hydrological_Model; % Runs the interception + infiltration + GW routing model

        if max(max(depths.d_t)) > 20000
            ttt = 1;
        end
        ...
        if flags.flag_D8 == 1 % D-8
            ...
            if flags.flag_inertial ~= 1 && flags.flag_CA == 1
                ...
                elseif flags.flag_kinematic == 1
                    ...
                    % -------------------- Local Inertial Formulation ----------------%
                    %                      [flow_rate.qout_left_t,flow_rate.qout_right_t,flow_rate.qout_up_t,flow_rate.qout_down_t,outlet_states.outlet_flow,depths.d_t,CA_States.I_tot_end_cell,outflow_bates,Hf,Qc,Qf,Qci,Qfi,C_a,LI_Work] = ...
                    ...
                    %                         flags.flag_reservoir,Elevation_Properties.elevation_cell,LI_Precomputed,LI_Work,...
