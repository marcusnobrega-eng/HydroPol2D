...
% Q_prev: previous outflow for channel (Qc_prev) and floodplain (Qf_prev) [m3/s]
...
% Qc_prev: previous channel outflow [m3/s]
% Qf_prev = previous floodplain outflow [m3/s]
% Qi_prev = inclined flows [m3/s]
% Q = floodplain outflow [m3/s]
% Qi = flows in inclined (NE, SE) directions [m3/s]
function [Q,Qc,Qf,Qci,Qfi] = subgrid_channel(h, w, zf, zc,dx, nc, nf, Qc_prev, Qf_prev,Qci_prev,Qfi_prev,g,dt,idx_rivers)
    
    nx = size(h,2);
    ny = size(h,1);

    yc = h + zc; % Channel head
