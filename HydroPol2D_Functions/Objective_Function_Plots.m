...
% Objective Function
function [Obj_function_value,NSE,r2,PBIAS,output_data] = Objective_Function_Plots(x,observed_concentration,time_observed_concentration)
% Please make sure to load the correct HydroPol2D model by loading the
% workspace with all inputs besides the rainfall intensity, C3, C4, h0, and
% d0.

%----- Files for 0.5deg ------%
% load workspace_05deg_24mmh
% i_conversion = 24.22; % mm/h

%----- Files for 2deg ------%
load workspace_2deg_62_61mmh
i_conversion = 62.61; % mm/h
...
% h0 and d0
h_0 = double(idx_not_nan)*x(8);
d_0 = double(idx_not_nan)*x(9);

% -- Rainfall 
delta_p = delta_p*(x(3)/i_conversion); % 24.22 is the rainfall intensity of the event used to save the workspace
% Watershed
[elevation,~,~] = plane_watershed(x(4),x(5),x(6),x(7),0);
obs_index_prev = 0;
...
% PBIAS
PBIAS = 100*(sum(observed_concentration' - modeled_concentration))/(sum(observed_concentration'));
...
figure(1)
close all
scatter(time_observed_concentration,modeled_concentration,'color','red')
hold on
scatter(time_observed_concentration,observed_concentration,'color','blue')
xlabel('Time (min)')
