...
while t < config.duration_s-time_tolerance
    ...
    if solver == "full_momentum"
        [surface_volume,hu,hv,edge_q,surface_diag,surface_boundary_diag] = Voronoi_Full_Momentum_Step( ...
            ...
            dry_tolerance_m=config.dry_tolerance_m);
    ...
    expected = initial_mass + sum(source_volume) + sum(groundwater_source_volume) + boundary_volume ...
        + groundwater_diag.fixed_head_exchange_m3 ...
    ...
    diagnostics(step).groundwater_boundary_exchange_m3 = groundwater_diag.fixed_head_exchange_m3;
    ...
    if isfinite(config.progress_interval_s) && t + time_tolerance >= next_progress
        ...
        if ~isempty(config.progress_checkpoint_file)
            ...
            percent=min(max(100*t/max(config.duration_s,eps),0),100);
            live = struct('stage','simulation','time_s',t,'step',step,'dt_s',dt, ...
                'simulated_minutes',t/60,'total_minutes',config.duration_s/60, ...
                'percent',percent,'time_step_seconds',dt, ...
...
if endsWith(lower(string(path)),".json")
    compact=rmfield(live,{'surface_depth_m','cfl'});
    hp2d_write_run_monitor(path,'',compact,false);
    return
end
...
values = struct('routing_solver','local_inertial','duration_s',3600,'initial_surface_depth_m',0, ...
    ...
    'initial_surface_momentum_y_m2_s',0, ...
    'initial_channel_discharge_m3_s',0, ...
    'initial_groundwater_head_m',0,'aquifer_bottom_m',-10,'hydraulic_conductivity_m_s',1e-5, ...
    'specific_yield',0.2,'groundwater_lateral_flow',true, ...
    'groundwater_fixed_head_mask',[],'groundwater_fixed_head_m',[], ...
    'compute_backend','cpu','output_netcdf','', ...
