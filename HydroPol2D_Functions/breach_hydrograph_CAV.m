...
function [time_min,flow_m3_s] = breach_hydrograph_CAV(h,W,Breach,dt,tf,flag_plot,ID,Name,depth_area,depth_volume)
...
for i = 1:length(time)
    ...
    if current_time <= tp
        ...
        if i == 1
            depth_m(i,1) = h0 - Qp*dt/(depth_area(h0));
        else
            depth_m(i,1) = h0 - Qp*dt/(depth_area(depth_m(i-1,1)));            
        end
    else
        flow_m3_s(i,1) = (-1/2*(Breach*sqrt(9.81))^(2/3)*h*(tp - current_time)/(Sf) + Qp^(-1/3))^(-3); % m3/s
        depth_m(i,1) = h0 - mean(flow_m3_s(i,1) + flow_m3_s(i-1,1))*dt/((depth_area(depth_m(i-1,1))));
    end
    h0 = depth_m(i,1);
end
