function [B_t,P_conc,Out_Conc,tmin_wq,tot_W_out,mass_lost,Tot_Washed] = build_up_wash_off_8D(C_3,C_4,qout_left_t,qout_right_t,qout_up_t,qout_down_t,outlet_flow,qout_ne_t,qout_se_t,qout_sw_t,qout_nw_t,B_t,time_step,nx_max,ny_max,cell_area,outlet_index,idx_nan_5,flag_wq_model,mass_lost,Tot_Washed,Bmin,Bmax,min_Bt)
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%                                                                 %
...
q_out_t = cat(3,qout_left_t,qout_right_t,qout_up_t,qout_down_t,outlet_flow,qout_ne_t,qout_se_t,qout_sw_t,qout_nw_t); % mm/h
large_timestep = 10; % seconds
idx_bt_1 = ((B_t*1000/cell_area < min_Bt) + isnan(B_t) + isinf(B_t)) > 0; % Values below 1 g / m2
idx_bt = repmat(idx_bt_1,1,1,9);
B_t_extra = B_t;
B_begin = B_t;
...
W_out_ne_t = W_out_t(:,:,6);
W_out_se_t = W_out_t(:,:,7);
W_out_sw_t = W_out_t(:,:,8);
W_out_nw_t = W_out_t(:,:,9);
...
zero_matrix = zeros(ny_max,nx_max);
W_in_ne_t = zero_matrix; W_in_se_t = zero_matrix; W_in_sw_t = zero_matrix; W_in_nw_t = zero_matrix;
W_in_ne_t(2:(ny_max),1:(nx_max-1)) = W_out_sw_t(1:(ny_max-1),2:(nx_max)); % OK
W_in_se_t(1:(ny_max-1),1:(nx_max-1)) = W_out_nw_t(2:ny_max,2:nx_max); % OK
W_in_sw_t(1:(ny_max-1),2:(nx_max)) = W_out_ne_t(2:(ny_max),1:(nx_max-1)); % OK
W_in_nw_t(2:ny_max,2:nx_max) = W_out_se_t(1:(ny_max-1),1:(nx_max-1)); % OK

W_in_t = cat(3,W_in_left_t,W_in_right_t,W_in_up_t,W_in_down_t,W_in_ne_t,W_in_se_t,W_in_sw_t,W_in_nw_t); % Total Inflow
% --------------- Net Pollutant Rate % ---------------%
