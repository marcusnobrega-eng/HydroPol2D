% HydroPol2D Optimization Function



% Read Right Input Maps

% Run Hydrodynamic Model

% Retrieve Hydrographs and Pollutographs
...
for ii = 1:Automatic_Calibrator.n_events
    ...
    if flags.flag_D8 ~= 1                                  
        [Qmod, Cmod,Qmod_gauges,Cmod_gauges] = HydroPol2D_Routing_Solver(t,time_obs,easting_obs_cell,northing_obs_cell,min_soil_moisture,Reservoir_Data,BC_States, CA_States, Courant_Parameters, date_begin, DEM_raster, depths, Elevation_Properties, flags, gauges, GIS_data, Human_Instability, Hydro_States, idx_nan, idx_nan_5, idx_outlet, Inflow_Parameters, LULC_Properties, Maps, nx_max, ny_max, outlet_index, outlet_runoff_volume, outlet_type, Rainfall_Parameters, recording_parameters, running_control, slope_outlet, Soil_Properties, t_previous, time_calculation_routing, time_step, time_step_model, tmin_wq, WQ_States, Wshed_Properties);
    else
        % Function for D8
    end
    ...
    if flag_outlet ~= 1
        if flags.flag_waterquality ~= 1
            if flag_opt_fun == 1 % NSE
                ...
                weights = 1/size(Qmod_gauges,2)*ones(1,size(Qmod_gauges,2));
                size_mod = size(Qmod_gauges,1);
                NSE = 1 - sum((obs_discharge(1:size_mod,:) - Qmod_gauges).^2)./(sum((obs_discharge(1:size_mod,:) - mean(Qmod_gauges)).^2)); % We want to maximize it               
                if sum(isnan(NSE))
            ...
            if flag_opt_fun == 2 % RMSE
                ...
                RMSE = sqrt(sum((obs_discharge(1:length(Qmod_gauges),:) - Qmod_gauges).^2/n_elements));
                Obj_fun(ii,1) = sum(weights.*RMSE); % minimize RMSE
            end

            if flag_opt_fun == 3 % R2
                r2 = corrcoef(obs_discharge(1:length(Qmod_gauges),:),Qmod_gauges);
                Obj_fun(ii,1) = -r2(1,2); % maximize r2
            end

            if flag_opt_fun == 4 % Peak Flow
                z1 = max(obs_discharge(1:length(Qmod_gauges),:));
                z2 = max(Qmod_gauges);
            ...
            if flag_opt_fun == 5 % NSE and Volume Error
                ...
                weights = 1/size(Qmod_gauges,2)*ones(1,size(Qmod_gauges,2));
                size_mod = size(Qmod_gauges,1);
                NSE = 1 - sum((obs_discharge(1:size_mod,:) - Qmod_gauges).^2)./(sum((obs_discharge(1:size_mod,:) - mean(Qmod_gauges)).^2) + small_number); % We want to maximize it               
                if sum(isnan(NSE))
                    error('Some of the NSE are becoming NaN.')
                end               
                % Volume Error
                VE = abs(sum((obs_discharge(1:size_mod,:) - Qmod_gauges))./(sum(obs_discharge(1:size_mod,:)) + small_number));
            ...
            if flag_opt_fun == 1 % NSE
                % Nash-Suctclife-Efficiency and Objective function
                NSE = 1 - sum((obs_pollutograph - Cmod_gauges).^2)/(sum((obs_pollutograph - mean(obs_pollutograph)).^2)); % We want to maximize it
                Obj_fun(ii,1) = -NSE; % Therefore, we want to minimize it
            end

            if flag_opt_fun == 2 % RMSE
                n_elements = length(obs_pollutograph);
                RMSE = sqrt(sum((obs_pollutograph - Cmod_gauges).^2/n_elements));
                Obj_fun(ii,1) = RMSE; % minimize RMSE
            end

            if flag_opt_fun == 3 % R2
                r2 = corrcoef(obs_pollutograph,Cmod_gauges);
                Obj_fun(ii,1) = -r2(1,2); % maximize r2
            end
