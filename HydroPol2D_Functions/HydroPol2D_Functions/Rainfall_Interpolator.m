...
function [Var_Interpolated,X,Y] = Rainfall_Interpolator(x_coordinate,y_coordinate,var_obs,x_grid,y_grid)
%% Meshgrid
...
F = var_obs; % Col Vector of rainfall intensities (mm/h)

Q1 = size(X,1);
Q2 = size(X,2);

Fint = idw_function([X1,Y1],F,X,Y); % Call idw_function
Var_Interpolated = reshape(Fint,Q1,Q2);
end