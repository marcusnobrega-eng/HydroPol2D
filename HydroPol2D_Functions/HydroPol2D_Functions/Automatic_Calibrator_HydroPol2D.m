...
flag_calibrate_wq =  table2array(initial_maps_data(1,5));
...
% Reading Input Data
observed_data = readtable('HydroPol2D_Automatic_Calibrator_Data.xlsx','Sheet','Observed_Data');
...
easting_observed_cells = round((-xulcorner + x_observed)/Resolution);
northing_observed_cells = round((yulcorner - y_observed)/Resolution);
...
%     routing_time = time_observed(n_observations,ii);   
    ...
    intensity_rainfall = intensity_rainfall*flag_rainfall;
    ...
    if flag_rainfall == 1 && flag_spatial_rainfall ~=1  % Only for concentrated rainfall
        ...
        for i =1:steps
            ...
            z1 = find(time_observed <= time,1,'last');
            if z1 > z2
