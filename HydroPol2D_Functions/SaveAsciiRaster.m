function SaveAsciiRaster(varname,Resolution,xllupcorner,yllupcorner,FileName,nodatavalue)
    % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
    %                                                                 %
...
PathName = pwd;
% OPEN file
fid = fopen(strcat(FileName,'.asc'),'w');
% fid = fopen('test.asc','w')
% if I loaded an xyz matrix as 'varname'

    cellsize = Resolution;
    hor = size(varname,2)*Resolution + Resolution;
    ver = size(varname,1)*Resolution + Resolution;
    ncols = size(varname,2);
    nrows = size(varname,1);
    raster = zeros(nrows,ncols);
    %create the header variable [default no data value is -9999]
    header = [ncols; nrows; xllupcorner; yllupcorner; cellsize; nodatavalue];
