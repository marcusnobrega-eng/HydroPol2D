...
elseif flags.flag_human_instability == 3
    ...
    for i = 1:length(unique)
        Human_Instability.order(find(temp==unique(i))) = i;
    end
else
    Human_Instability = [];
end
...
if flags.flag_reservoir == 1
	...
	Reservoir_Data.bculv = table2array(input_table(:,7));
	Reservoir_Data.hculv = table2array(input_table(:,8));
	Reservoir_Data.Lculv = table2array(input_table(:,9));
	Reservoir_Data.nculv = table2array(input_table(:,10));
	Reservoir_Data.So = table2array(input_table(:,11));
	Reservoir_Data.Ke = table2array(input_table(:,12));
	Reservoir_Data.Ks = table2array(input_table(:,13));
    Reservoir_Data.Dir = table2array(input_table(:,14));
    Reservoir_Data.Qreg = table2array(input_table(:,15));
    % Calculating Kv = Cd * Lef and culvert parameters
    Reservoir_Data.Kv = Reservoir_Data.Cd.*Reservoir_Data.Lef;
    Reservoir_Data.A_max = Reservoir_Data.bculv.*0.9.*Reservoir_Data.hculv;
	Reservoir_Data.Kculv = Reservoir_Data.Ke+Reservoir_Data.Ks;
	Reservoir_Data.Rhculv_max = Reservoir_Data.bculv.*0.9.*Reservoir_Data.hculv./(Reservoir_Data.bculv+(2.*0.9.*Reservoir_Data.hculv));
