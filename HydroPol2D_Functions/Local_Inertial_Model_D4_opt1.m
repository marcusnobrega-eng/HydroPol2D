function [qout_left,qout_right,qout_up,qout_down,outlet_flow,d_t,I_tot_end_cell,outflow,Hf,Qc,Qf,Qci,Qfi,C_a,LI_Work] = ...
    Local_Inertial_Model_D4_opt1(flag_numerical_scheme,reservoir_x,reservoir_y,k1,h1,k2,k3,h2,k4,yds1,xds1,yds2,xds2,...
    flag_reservoir,z,LI_Precomputed,LI_Work,...
    d_tot,d_p,roughness_cell,cell_area,time_step,Resolution,outlet_index,outlet_type,slope_outlet,row_outlet,col_outlet,...
    d_tolerance,outflow,idx_nan,flag_critical,flag_subgrid,nc,nf,River_Width,River_Depth,Qc_prev,Qf_prev,Qci_prev,Qfi_prev,C_a_prev,...
    Subgrid_Properties,flag_overbanks,flag_inflow,SubgridTables)% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%                                                                 %
...
% ---- X faces (i+1/2, j)
zf_x = LI_Precomputed.zf_x;                              % bed at the face
...
% ---- Y faces (i, j+1/2)
zf_y = LI_Precomputed.zf_y;                              % bed at the face
...
matrix_store = LI_Work.matrix_store;        % ny×nx×2
matrix_store(:,:,1)= Sx;               % Sx at (i+1/2, j)
matrix_store(:,:,2)= Sy;               % Sy at (i, j+1/2)

Hf           = LI_Work.Hf;               % ny×nx×2
...
LI_Work.matrix_store = matrix_store;
LI_Work.Hf = Hf;
LI_Work.outflow = outflow;
end
