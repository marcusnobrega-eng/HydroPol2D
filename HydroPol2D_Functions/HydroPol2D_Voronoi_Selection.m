...
if strcmpi(run_mode,'bypass')
    ...
    settings = HydroPol2D_Read_Voronoi_Settings(input_excel_file,general);
    selection.case_file=char(string(cell_right(settings,'Voronoi case file','')));
    selection.options=struct( ...
        'background_target_width_m',cell_right(settings,'Background width',2000), ...
        'minimum_cell_width_m',cell_right(settings,'Minimum cell width',100), ...
        'maximum_adjacent_size_ratio',cell_right(settings,'Adjacent size ratio',2), ...
        'urban_target_width_m',cell_right(settings,'Urban target width',200), ...
        'urban_transition_buffer_m',cell_right(settings,'Urban buffer',1000), ...
        'unresolved_river_policy',char(string(cell_right(settings,'Unresolved rivers','neal_subgrid'))), ...
        'subgrid_table_path',char(string(cell_right(settings,'Voronoi subgrid table',''))));
    selection.output_controls=struct( ...
        'write_native_archive',logical(cell_right(settings,'Write native archive',1)), ...
        'native_output_interval_s',cell_right(settings,'Native output interval s',300), ...
        'write_final_geotiffs',logical(cell_right(settings,'Write final GeoTIFFs',1)), ...
        'write_temporal_geotiffs',logical(cell_right(settings,'Write temporal GeoTIFFs',1)), ...
        'raster_stack_interval_s',cell_right(settings,'Raster stack interval s',3600), ...
        'write_figures',logical(cell_right(settings,'Write figures',1)), ...
        'write_videos',logical(cell_right(settings,'Write videos',1)), ...
        'write_gauge_hydrographs',logical(cell_right(settings,'Write gauge hydrographs',1)));
end
...
tf = isempty(value) || (isscalar(value) && ismissing(value));
if ~tf && (ischar(value) || isstring(value))
