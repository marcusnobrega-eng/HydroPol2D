%@ -0,0 +1,242 @@
function [qout_left,qout_right,qout_up,qout_down,outlet_flow,qout_ne,qout_se,qout_sw,qout_nw,d_t,I_tot_end_cell,outflow] = Bates_Inertial_8D(reservoir_dir,reservoir_x,reservoir_y,Kv,pv,flag_reservoir,elevation_cell,d_tot,roughness_cell,cell_area,time_step,Resolution,I_tot_end_cell,outlet_index,outlet_type,slope_outlet,row_outlet,col_outlet,Ko,po,d_tolerance,outflow,idx_nan)
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%                                                                 %
...
depth_cell(depth_cell <= h_min) = 1e-12;
...
outflow = (outflow_prev - 9.81*depth_cell*dt.*matrix_store)./(1 + 9.81*depth_cell*dt.*roughness_cell.^2.*outflow_prev./(depth_cell.^(10/3))); % m2 per sec
