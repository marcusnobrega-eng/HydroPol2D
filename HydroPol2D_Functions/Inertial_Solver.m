...
if flag_numerical_scheme == 2
    ...
    q_flux(:,2:(end),2) = q_p(:,1:(end-1),2);          % Right


    %%% y-y direction
    q_flux(1:(end-1),:,3) = q_p(2:(end),:,3);          % Up
    q_flux(2:(end),:,4) = q_p(1:(end-1),:,4);          % Down 

    %%% x-x direction
    q_flux_(:,2:(end),1) = q_p(:,1:(end-1),1);          % Left
    q_flux_(:,1:(end-1),2) = q_p(:,2:(end),2);          % Right


    %%% y-y direction
    q_flux_(2:(end),:,3) = q_p(1:(end-1),:,3);          % Up
    q_flux_(1:(end-1),:,4) = q_p(2:(end),:,4);          % Down     

    q_flux(mask) = q_flux_(mask);
    ...
    %%% Effect of borders
    % theta(1,:,1:5) = 1; theta(size(q_p,1),:,1:5) = 1; 
    % theta(:,1,1:5) = 1; theta(:,size(q_p,2),1:5) = 1;
