...
elseif flag_subgrid == 1 && flag_overbanks ~= 1 && ~isempty(SubgridTables)

    % Shared-face / lookup-table subgrid:
    % volume is obtained from the prestored cell volume table.
    eta_now = z + depth_cell;
    ...
    current_volume = nansum(V_now(:));

else

    if isscalar(C_a_prev)
        current_volume = nansum(nansum(C_a_prev * depth_cell));
    else
        current_volume = nansum(nansum(C_a_prev .* depth_cell));
    end
...
elseif flag_subgrid == 1
    ...
    % Previous discharge per unit width [m2/s]
    q_prev = outflow_prev(:,:,1:2);

    % Compute subgrid-aware face discharge using:
    %   Hf = face_area / Resolution
    %   n  = representative face n from SubgridTables.n_x / n_y
    [outflow, Hf_x, Hf_y, Wf_x, Wf_y] = subgrid_topography_model( ...
        flag_numerical_scheme, ...
        y, ...
        q_prev, ...
        nc, ...
        Resolution, ...
    ...
    % Effective cell width for outlet logic only
...
if flag_subgrid == 1 && flag_overbanks ~= 1 && ~isempty(SubgridTables)

    % Shared-face / lookup-table subgrid:
    % update volume, then invert V -> eta using the subgrid volume table.

    eta_now = z_dem + depth_cell;
    ...
    % Updated wetted cell area for mass balance / outlet diagnostics
    C_a = hp2d_lookup_eta_uniform( ...
        SubgridTables.area_cell, ...
        eta_new, ...
        SubgridTables.invert_el, ...
...
%==========================================================================
% LOOKUP TABLE BY ABSOLUTE WATER SURFACE ELEVATION
%==========================================================================
% PURPOSE
%   Interpolate a subgrid table using absolute water surface elevation:
%
%       depth = eta - invert_el
%
% INPUTS
%   Vtab      [ny x nx x nz] lookup table
%   eta       [ny x nx]      absolute water surface elevation [m]
%   invert_el [ny x nx]      local invert elevation [m]
%   dz        scalar         vertical spacing [m]
...
%
% NOTES
%   Invalid table cells remain NaN.
%   Above maxDepth, volume is extended as a full-cell prism.
%==========================================================================
...
% Return underlying numeric class for regular arrays or gpuArray.
