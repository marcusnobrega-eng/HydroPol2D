...
% - If flag_resample = 1, define:
%       InputData_Bypass.general.resolution_resample
%   in meters.
%   This value is used later in preprocessing and input handling for raster
%   resampling, inlet coordinate conversion, and related area calculations.
% ========================================================================
...
if ~exist('InputPaths','var') || ~isstruct(InputPaths)
    error(['InputPaths was not found in the workspace. ' ...
           'Run input_paths_bypass_script.m before input_data_bypass_script.m']);
end
...
InputData_Bypass.general.min_time_step        = 1/6000; % [min] Example minimum adaptive step
InputData_Bypass.general.max_time_step        = 5;      % [min] Maximum allowed adaptive step
InputData_Bypass.general.time_step_increments = 0.001;  % [min] Legacy increment used internally
InputData_Bypass.general.time_step_change     = 0.10;   % [-] Legacy adaptive change factor

% -------------------------------------------------------------------------
% Courant / stability controls
% -------------------------------------------------------------------------
InputData_Bypass.general.alfa_min = 0.30;   % [-]
...
InputData_Bypass.general.date_begin = datetime(2025,6,1,0,0,0);
InputData_Bypass.general.date_end   = datetime(2025,7,1,0,0,0);
...
%   MSWEP 3-hourly      -> 180
%   Daily ETP maps      -> 1440
InputData_Bypass.general.dt_rainfall_maps_min      = 180;   % [min]
...
% -------------------------------------------------------------------------
InputData_Bypass.general.slope_outlet                  = 0.001; % [m/m]
InputData_Bypass.general.n_outlets_data                = 1;     % legacy default usually 1
InputData_Bypass.general.record_time_maps              = 60;    % [min]
InputData_Bypass.general.record_time_hydrographs       = 5;     % [min]
...
InputData_Bypass.general.record_time_spatial_rainfall  = 60;    % [min]
...
% -------------------------------------------------------------------------
% DEM / grid preprocessing controls
% Includes smoothing, stream extraction thresholds, and optional target
% resolution used when flag_resample = 1.
% -------------------------------------------------------------------------
...
InputData_Bypass.general.resolution_resample = 250; % [m] target model grid size when flag_resample = 1
...
LULC_table.Index = [10;20;30;40;50;60;70;80;90;95;100];
...
% ========================================================================
% Uses raster files discovered and sorted in input_paths_bypass.m, then
% keeps only the rasters that fall inside the simulation period defined by:
%
...
% ------------------------------------------------------------------------
% The simulation dates are defined in this bypass input-data script, not in
% input_paths_bypass.m. Therefore, the rainfall subset must be determined
% here, after date_begin and date_end are known.
%
% PERFORMANCE STRATEGY
% ------------------------------------------------------------------------
% If the rainfall folder contains many years of data, parsing every single
% filename can be slow. To reduce the cost:
%
%   1) first prefilter by YEAR using cheap string matching
%   2) then parse datetime only for the reduced subset
%
% REQUIRED FILENAME FORMAT
% ------------------------------------------------------------------------
% Rainfall rasters must contain a recognizable date/time in the filename,
% such as:
%
%   IMERG_2026_01_01_00_00.tif
%   IMERG_2026-01-01-03-00.tif
%   Rain_2026_01_15.tif
%
% Supported patterns:
%   YYYY_MM_DD_HH_mm
...
% -------------------------------------------------------------------------
% Full rainfall file list already discovered in input_paths_bypass.m
% -------------------------------------------------------------------------
rain_files_all = InputPaths.Rainfall_Raster_Files;
...
% -------------------------------------------------------------------------
% Cheap prefilter by year
% -------------------------------------------------------------------------
% This avoids parsing dates from every filename when the folder contains
% many years of rasters.
year_begin = year(date_begin_sim);
year_end   = year(date_end_sim);

keep_year = false(numel(rain_files_all),1);

for i = 1:numel(rain_files_all)
    [~, fname_only, ext_only] = fileparts(rain_files_all{i});
    fname_full = [fname_only ext_only];

    for yy = year_begin:year_end
        if contains(fname_full, sprintf('%04d', yy))
            keep_year(i) = true;
            break
        end
    end
...
rain_files_candidate = rain_files_all(keep_year);

% -------------------------------------------------------------------------
% Exact filtering by datetime
% -------------------------------------------------------------------------
rain_files = {};
rain_times = datetime.empty(0,1);

for i = 1:numel(rain_files_candidate)
    this_file = rain_files_candidate{i};
    [~, this_name, this_ext] = fileparts(this_file);

    % Parse timestamp from filename
    this_dt = parse_datetime_from_filename_local([this_name this_ext]);

    if isnat(this_dt)
        error(['Could not parse datetime from rainfall raster filename:\n  %s\n' ...
               'Please use names such as YYYY_MM_DD_HH_mm or YYYY-MM-DD-HH-mm.'], ...
               this_file);
    end

    if this_dt >= date_begin_sim && this_dt <= date_end_sim
        rain_files{end+1,1} = this_file; %#ok<AGROW>
        rain_times(end+1,1) = this_dt;   %#ok<AGROW>
    end
end

% -------------------------------------------------------------------------
% Build rainfall schedule
% -------------------------------------------------------------------------
n_rain = numel(rain_files);

if n_rain > 0
    dt_rain = InputData_Bypass.general.dt_rainfall_maps_min;

    % Sort again after filtering just to be safe
    [rain_times, idx_sort] = sort(rain_times);
    rain_files = rain_files(idx_sort);

    Input_Rainfall.time = (0:n_rain-1)' * dt_rain;
    Input_Rainfall.labels_Directory = cellfun(@char, rain_files, 'UniformOutput', false);
    Input_Rainfall.num_obs_maps = n_rain;

    fprintf('[Rainfall filtering] %d raster(s) retained between %s and %s.\n', ...
        n_rain, char(string(date_begin_sim)), char(string(date_end_sim)));

    fprintf('[Rainfall filtering] First raster time: %s\n', char(string(rain_times(1))));
    fprintf('[Rainfall filtering] Last raster time : %s\n', char(string(rain_times(end))));
    ...
    warning(['No rainfall rasters were found inside the simulation window:\n' ...
             '  Start: %s\n  End:   %s'], ...
             char(string(date_begin_sim)), char(string(date_end_sim)));
end
