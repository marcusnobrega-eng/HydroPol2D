...
if flag_control == 1 && flag_reservoir == 1
%identify the point and extract the value of the flow 
%identify the base Flow - given as an input info
%compare the base flow with the flow at the point
	% verify if the levels in the entrance points allow to modify the parameters
	%_key curve at the enctrance to see those limit conditions
		%change the parameters k1 
			%to establish the change we need the keycurve in the control point, that lineraized the problem.  
			%Reference point in the entrance of the culvert or spillway, for the capacity of the structures 
			%establish the limit of openess of orifice and spillway
