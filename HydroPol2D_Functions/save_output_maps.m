...
if flags.flag_automatic_calibration ~= 1
    if k == 1 % First time-step
        ...
        if flags.flag_snow_modeling == 1
            Maps.Hydro.Snowpack(:,:,1) = Snow_Properties.H_snow_t; % Snowpack considering its density [mm]
        end
    ...
    elseif recording_parameters.delta_record > 0 % Saving maps
        ...
        % Snowpack
        if flags.flag_snow_modeling == 1
            Maps.Hydro.Snowpack(:,:,saver_count) = Snow_Properties.H_snow_t; % Snowpack considering its density [mm]
        end
