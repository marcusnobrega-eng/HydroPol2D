...
InputData_Bypass.general.min_time_step        = 0.001;      % [sec] Example minimum adaptive step
InputData_Bypass.general.max_time_step        = 1;  % [sec] Maximum allowed adaptive step
InputData_Bypass.general.time_step_increments = 0.001;  % [sec] Legacy increment used internally
InputData_Bypass.general.time_step_change     = 0.001;   % [-] Legacy adaptive change factor
...
InputData_Bypass.general.date_end   = datetime(2025,5,1,2,30,0);
...
%   MSWEP 3-hourly      -> 180
%   Daily ETP maps      -> 1440
InputData_Bypass.general.dt_rainfall_maps_min      = 10/60;   % [min]
...
InputData_Bypass.general.n_outlets_data                = 2;     % legacy default usually 1
InputData_Bypass.general.record_time_maps              = 1;    % [min]
InputData_Bypass.general.record_time_hydrographs       = 60;     % [min]
...
InputData_Bypass.general.record_time_spatial_rainfall  = 10/60;    % [min]
...
% -------------------------------------------------------------------------
InputData_Bypass.general.min_area  = 0.01;      % km2
...
InputData_Bypass.flags.flag_stage_hydrograph             = 1;
...
InputData_Bypass.flags.flag_numerical_scheme             = 2;
...
%
% IMPORTANT
%   The raster-like gete variables below depend on DEM_raster already
...
% ========================================================================
% Non-breaking wave analytical stage boundary condition.
%
% This is ONE boundary condition applied to all inlet cells.
%
% Boundary:
%   h(0,t) = [(7/3) * n^2 * u^3 * t]^(3/7)
%
% IMPORTANT:
%   This is stage/depth [m], NOT discharge.
%   Therefore, do NOT divide by the number of cells.
% ========================================================================
...
dx_nb = 25;          % [m]
ny_nb = 32;          % number of inlet cells
nx_nb = 240;
...
% Time vector
t_end_min    = 150;  % 9000 s
dt_stage_min = 1/60;    % [min]

time_min = (0:dt_stage_min:t_end_min)';
time_sec = time_min * 60;

% Analytical stage hydrograph
H_stage_m = ((7/3) * n_nb^2 * u_nb^3 .* time_sec).^(3/7);
H_stage_m(time_sec == 0) = 0;

% Coordinates of the inlet boundary cells
% Use CELL CENTERS, not x = 0 edge.
% This avoids ambiguity in raster coordinate-to-index conversion.

x_inlet_cells = (dx_nb/2) * ones(ny_nb,1);  % first-column centers

y_inlet_cells = Ly_nb - ((1:ny_nb)' - 0.5) * dx_nb;

% ONE stage hydrograph applied to MANY cells
Stage.time_stage = time_min;      % [min]
Stage.stage      = H_stage_m;     % one hydrograph only [m]

% One boundary condition, but multiple cells where it is applied
Stage.x_coords = {x_inlet_cells};
Stage.y_coords = {y_inlet_cells};

% Optional CSV export for inspection
Stage_Table = table();
Stage_Table.time_min = time_min;
Stage_Table.H_1      = H_stage_m;

% Store coordinates as separate diagnostic table
Stage_Coordinates = table();
Stage_Coordinates.Cell_ID = (1:ny_nb)';
Stage_Coordinates.x_m = x_inlet_cells;
Stage_Coordinates.y_m = y_inlet_cells;

try
    stage_out_dir = fullfile('/oak/stanford/groups/gorelick/HydroPol2D/Case_Studies/NonBreakingWave/Static');

    if ~exist(stage_out_dir,'dir')
        mkdir(stage_out_dir);
    end

    writetable(Stage_Table, ...
        fullfile(stage_out_dir, 'NonBreakingWave_Stage_Hydrograph.csv'));

    writetable(Stage_Coordinates, ...
        fullfile(stage_out_dir, 'NonBreakingWave_Stage_Coordinates.csv'));

catch
    warning('Could not export NonBreakingWave stage files.');
end
