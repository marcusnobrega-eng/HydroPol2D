# VAL-SNOW-001: Configurable Snow Accumulation and Melt

This validation suite verifies the active snow pathway in HydroPol2D. Snow
...
| Case | Test | Evidence |
|---|---|---|
| `VAL-SNOW-001A` | Configurable cold, mixed, and warm partition | Exact prescribed partition and snow-storage balance |
| `VAL-SNOW-001B` | Per-LULC parameter maps and initial snow states | No-raster, SWE-only, depth-only, both-raster, and inconsistent-raster paths |
| `VAL-SNOW-001C` | Timestep refinement | 5, 15, and 60 min integrations of the same degree-day solution |
| `VAL-SNOW-001D` | Controlled V-tilted snow/runoff coupling | Closed liquid-water and snow-storage ledger on a two-LULC grid |
| `VAL-SNOW-001E` | Input-surface equivalence | Identical parameter arrays from Excel and bypass configuration |
| `VAL-SNOW-001F` | Normal HydroPol2D V-tilted event | Regular preprocessing, meteorological forcing, snow routing, and event mass ledger |
...
```matlab
run('Validation/Snow/ColdWarmPartition_Melt/run_snow_validation.m')
```
...
timeseries, figures, and ledger below `FullModelRuns/VAL-SNOW-001F/`.
