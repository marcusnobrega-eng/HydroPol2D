...
saved_depth = double(ncread(output_file,'surface_depth_face_m'));
saved_velocity = double(ncread(output_file,'surface_velocity_face_m_s'));
saved_volume = double(ncread(output_file,'final_surface_volume_face_m3'));
...
assert(strcmp(ncreadatt(output_file,'/','output_schema'),'hydropol2d-unstructured-output-1.0'));
assert(isequal(double(ncread(output_file,'map_time_s')),results.map_time_s));
