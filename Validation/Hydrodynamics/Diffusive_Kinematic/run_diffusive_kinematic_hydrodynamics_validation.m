...
writetable(Diagnostics, fullfile(out_dir, 'Diffusive_Kinematic_Hydrodynamics_Diagnostics.csv'));
writetable(PassFail, fullfile(out_dir, 'Diffusive_Kinematic_Hydrodynamics_Pass_Fail.csv'));
...
if method == "kinematic"
    passed = mass_abs_pct < 0.1 && nse > 0.90 && rel_l2 < 0.25 && peak_q_error_pct < 10;
else
    passed = mass_abs_pct < 0.1 && max(q_model_m3_s) > 0 && nse > 0.50;
end
...
[h, state, q_out_m3_s] = wave_step(method, h, z, roughness, dt_s, dx, state, Outlet, outlet_slope);
state.q_out_m3_s = q_out_m3_s;
end

function [h, state, q_out_m3_s] = wave_step(method, h, z, roughness, dt_s, dx, state, Outlet, outlet_slope)
...
if method == "kinematic"
    [~,~,~,~,outlet_flow,d_t,state.I_tot_end_cell,state.outflow] = Kinematic_Wave_Model( ...
        1, [], [], [], [], [], [], [], [], [], [], [], [], ...
        0, z, d_tot, d_p, roughness, cell_area, time_step_min, dx, ...
        Outlet.mask, 1, outlet_slope, Outlet.row, Outlet.col, d_tolerance_mm, ...
        state.outflow, idx_nan, 0, 0, [], [], River_Width, River_Depth, ...
        0, 0, 0, 0, cell_area);
elseif method == "diffusive"
    [~,~,~,~,outlet_flow,d_t,state.I_tot_end_cell,state.outflow] = Diffusive_Wave_Model_Implicit( ...
        1, [], [], [], [], [], [], [], [], [], [], [], [], ...
        0, z, d_tot, d_p, roughness, cell_area, time_step_min, dx, ...
        Outlet.mask, 1, outlet_slope, Outlet.row, Outlet.col, d_tolerance_mm, ...
        state.outflow, idx_nan, 0, 0, [], [], River_Width, River_Depth, ...
...
Cfg.case_id = "VAL-HYDRO-" + prefix + "-" + suffix;
if case_key == "stage"
    Cfg.case_id = "VAL-BC-STAGE-" + prefix + "-001";
end
...
if method == "kinematic"
    method_note = "Kinematic-wave analytical metrics use the tilted-plane kinematic-wave solution as the primary benchmark; Ritter remains diagnostic because the equation does not solve momentum.";
else
    method_note = "Diffusive-wave analytical plane metrics are approximation benchmarks; Ritter remains diagnostic because the equation omits full dynamic-wave inertia.";
end
notes = [ ...
    method_note; ...
...
exportgraphics(fig, fullfile(fig_dir, "VAL_HYDRO_" + prefix + "_PLANE_001.png"), 'Resolution', 200);
close(fig);
...
xlabel('Time (min)');
exportgraphics(fig, fullfile(fig_dir, "VAL_HYDRO_" + prefix + "_VTILT_001.png"), 'Resolution', 200);
close(fig);
...
xlabel('x (m)');
exportgraphics(fig, fullfile(fig_dir, "VAL_HYDRO_" + prefix + "_RITTER_001.png"), 'Resolution', 200);
close(fig);
...
xlabel('Time (min)'); ylabel('Mass error (%)');
exportgraphics(fig, fullfile(fig_dir, "VAL_BC_STAGE_" + prefix + "_001.png"), 'Resolution', 200);
close(fig);
end
