...
application_dir = fileparts(mfilename('fullpath'));
model_root = fileparts(fileparts(application_dir));
if ~isfolder(fullfile(model_root, 'HydroPol2D_Functions'))
    error('HydroPol2D:SNISB:ModelRootNotFound', ...
        'Could not locate the HydroPol2D repository root from: %s', application_dir);
end
config_dir = fullfile(model_root, 'Config');

input_paths_function = fullfile(config_dir, 'input_paths_bypass.m');
input_data_bypass_script_path = fullfile(application_dir, 'input_data_bypass_snisb.m');
