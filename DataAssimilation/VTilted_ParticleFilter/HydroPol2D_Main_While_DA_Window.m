...
% clear all
% load workspace_subgrid_90m.mat
...
if ~exist('DA_Initialized', 'var') || isempty(DA_Initialized) || ~DA_Initialized
...
if flags.flag_full_momentum == 1 && (~exist('DA_MomentumInitialized', 'var') || isempty(DA_MomentumInitialized) || ~DA_MomentumInitialized)
...
if flags.flag_full_momentum == 1 && (~exist('Outlet_Properties', 'var') || isempty(Outlet_Properties))
...
profile on
if ~exist('DA_window_end_h', 'var') || isempty(DA_window_end_h)
    DA_window_end_h = running_control.routing_time;
end
while t <= (DA_window_end_h + running_control.min_time_step/60) % Running up to the requested DA window
    %tic
    try
...
DA_is_final_window = DA_window_end_h >= (running_control.routing_time - 1.0e-9);

if DA_is_final_window
