...
for run in runs:
    ...
    if run == runs[0]:
         ...
         axs_r = axs.twinx()
         axs_r.bar(obs['DATA'].astype(str).str[5:-3], obs['Unnamed: 3'], color='dodgerblue', width=1, alpha=0.7, label="$Rainfall$")
         ...
         axs_r = axs.twinx()
         axs_r.bar(obs['DATA'].astype(str).str[5:-3], obs['Unnamed: 3'], color='dodgerblue', width=1, alpha=0.7)
    ...
    axs_r.set_ylim([0, obs['Unnamed: 3'].max() * 3])
    axs_r.invert_yaxis()
